<template>
    ...
    <div class="flex flex-wrap gap-2">
        <button v-for="(label, index) in splitLabels" :key="index" @click="updateLabel(label)"
            :class="currentLabel === label ? 'bg-gray-500 text-white' : 'bg-white text-black'"
...
const splitLabels = computed(() => {
    return props.suggestedLabels ? props.suggestedLabels.split(',').map(label => label.trim()) : [];
});
</script>